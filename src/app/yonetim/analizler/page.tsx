    ...
    <div className="p-6 bg-background min-h-screen text-text-main font-body">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-xs text-text-muted font-body mt-1">Laboratuvardan gelen bal saflık ve prolin analiz sonuçlarını girin.</p>
        </div>
      ...
      {isAdding && (
        <div className="bg-white border-gray-200 shadow-sm backdrop-blur-xl p-6 rounded-2xl border border-gray-200 mb-8 shadow-xl">
          ...
          <form onSubmit={handleSave} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-text-muted mb-1 text-xs uppercase font-bold">Parti No / QR Kodu (Benzersiz)</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full p-3 bg-gray-50 border-gray-200/50 border border-gray-200 rounded-lg text-xs text-secondary outline-none focus:border-primary disabled:opacity-50"
                />
              </div>
              <div>
                <label className="block text-text-muted mb-1 text-xs uppercase font-bold">Ürün Adı</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-text-muted mb-1 text-xs uppercase font-bold">Analiz Tarihi</label>
                <input
                  type="date"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-text-muted mb-1 text-xs uppercase font-bold">Prolin Değeri (mg/kg)</label>
                <input
                  type="number"
                  step="0.1"
                  required
              ...
              <div>
                <label className="block text-text-muted mb-1 text-xs uppercase font-bold">Nem Oranı (%)</label>
                <input
                  type="number"
                  step="0.1"
                  required
              ...
              <div>
                <label className="block text-text-muted mb-1 text-xs uppercase font-bold">Diastaz Sayısı</label>
                <input
                  type="number"
                  step="0.1"
                  required
            ...
            <div className="grid grid-cols-1 gap-4">
              <div>
                <label className="block text-text-muted mb-1 text-xs uppercase font-bold">Laboratuvar Belgesi Resim URL</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-text-muted mb-1 text-xs uppercase font-bold">Lab Notları & Yorumlar</label>
                <textarea
                  placeholder="Balın kalitesi ve laboratuvar sonuçları hakkında notlar girin..."
                  value={formData.notes}
                  onChange={e => setFormData({...formData, notes: e.target.value})}
                  className="w-full p-3 bg-gray-50 border-gray-200/50 border border-gray-200 rounded-lg text-xs text-secondary outline-none focus:border-primary min-h-[80px]"
                />
              </div>
            </div>
              ...
              <tbody className="divide-y divide-gray-800/50">
                {analyses.map(an => (
                  <tr key={an.id} className="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="p-4 font-heading font-bold text-secondary uppercase">{an.productName}</td>
