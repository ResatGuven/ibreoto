...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!post) {
    return {
      title: 'Yazı Bulunamadı - Arı Hayat',
      description: 'Aradığınız blog yazısı bulunamadı.'
    };
  }

  return {
    title: `${post.title} - Arı Hayat Blog`,
    description: post.excerpt || `${post.title} hakkında detaylı bilgi, bilimsel çalışmalar ve sağlıklı reçeteler blog yazımızda.`,
    openGraph: {
      title: post.title,
